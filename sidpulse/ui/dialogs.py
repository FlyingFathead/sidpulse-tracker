...
def choices(dialog):
    ...
    if 'discard' in dialog:return [(dialog.get('save_label','Save & Quit' if dialog.get('quit') else 'Save'),pg.K_s),(dialog.get('discard_label','Discard & Quit' if dialog.get('quit') else 'Discard'),pg.K_d),('Cancel',pg.K_ESCAPE)]
    ...
    if 'yes' in dialog:return [(dialog.get('confirm_label','OK'),pg.K_y),('Cancel',pg.K_ESCAPE)]
